...
var LEVELS = [
  ...
  },
  {
    ...
    goal:'ตอนตัดไฟ ขดลวดมอเตอร์จะเหวี่ยงแรงดันย้อนกลับสูงเป็นสิบเท่าของแหล่งจ่าย ต่อวงหลักให้มอเตอร์หมุนก่อน แล้วเติมสายอีก 2 เส้นเอาไดโอด "คร่อมขามอเตอร์แบบกลับขั้ว" (ขา − ของไดโอดไปข้างขั้วบวกของมอเตอร์) จากนั้นลองสับสวิตช์ OFF เทียบกับด่านที่แล้วดู',
    ...
    /* วงหลัก 3 เส้น แล้วคร่อมไดโอดทับขามอเตอร์อีก 2 เส้น แบบกลับขั้ว
       (diode.right = ขา − / คาโทด ไปที่ motor.left = ขั้วบวกของมอเตอร์) */
    solution:[["battery_9v.right", "switch.left"], ["switch.right", "motor.left"], ["motor.right", "battery_9v.left"],
              ["motor.left", "diode.right"], ["motor.right", "diode.left"]],
    topology:{type:'series'},
    /* ผังคำใบ้วาดแค่วงหลัก แล้วอธิบายสายคร่อมด้วยข้อความ
       เพราะ solutionChain() ไล่เฉลยเป็นโซ่เส้นเดียว วาดสาขาคร่อมไม่ได้ */
    hintChain:['battery_9v','switch','motor'],
    hintNote:'ต่อวงหลักเรียงกันเป็นวงเดียวก่อน แล้วเติมสายอีก 2 เส้นคร่อมขามอเตอร์: ขา − ของไดโอด → ขั้วบวกของมอเตอร์ และขา + ของไดโอด → ขั้วลบของมอเตอร์',
    /* ไม่ใช้ topology:'series' แล้ว เพราะไดโอดที่คร่อมขดลวดทำให้มี 2 สาขาตามจริง
       และไม่ใช้ conducting:['diode'] เพราะไดโอดคายพลังงานที่ต่อถูก
       "ต้องไม่นำกระแส" ตอนทำงานปกติ — เงื่อนไขเดิมกลับหัวกับความจริง */
    require:{work:['motor'], switchControls:true, flyback:true},
    ...
    tutorial:[
      ...
      {img:'Insert_image_here',text:'ต่อไว้ตรงไหน? (ข้อนี้สำคัญที่สุด)\n\nไดโอดต้องต่อ "คร่อมขาของมอเตอร์" ไม่ใช่ต่ออนุกรม\nและต้องหันขั้วกลับด้านกับแหล่งจ่าย:\n\n  ขา − (ฝั่งขีด · คาโทด)   → ข้างขั้วบวกของมอเตอร์\n  ขา + (ฝั่งสามเหลี่ยม)      → ข้างขั้วลบของมอเตอร์\n\nตอนไฟเข้าปกติ ไดโอดถูกไบแอสย้อน = ไม่นำกระแสเลย\nไม่กินไฟ ไม่กวนวงจร มอเตอร์หมุนเหมือนไม่มีมันอยู่\n\nพอสับสวิตช์ OFF แรงดันคร่อมขดลวดกลับขั้วทันที\nไดโอดจึงนำกระแส เกิดเป็นวงปิดเล็ก ๆ\nระหว่างไดโอดกับมอเตอร์ ให้กระแสวนอยู่ในนั้น\nจนหมดแรงไปกับความต้านทานของขดลวดเอง'},
      {img:'Insert_image_here',text:'ทำไมต่ออนุกรมแทนไม่ได้?\n\nไดโอดที่วางอนุกรมในวงหลัก\nจะขาดไปพร้อมวงจรตอนสวิตช์เปิด\nกระแสในขดลวดจึงไม่มีทางวนเลย\nแรงดันย้อนก็ยังพุ่งออกมาเท่าเดิม\n\nต้องเป็นวงปิดที่ "ไม่ผ่านสวิตช์" เท่านั้น\nจึงต้องคร่อมขามอเตอร์ตรง ๆ\n\nและจำไว้อีกข้อ: ไดโอดกันได้แค่แรงดันพุ่งตอนตัดไฟ\nไม่ได้ลดแรงดันที่จ่ายเข้าตลอดเวลา\nถ้าจ่ายไฟเกินพิกัด มอเตอร์ก็ยังไหม้อยู่ดี'},
    ],
    check:levelCheck({
      need:[['motor','ยังขาด มอเตอร์'],['diode','ต้องใส่ไดโอดป้องกัน Back-EMF!']],
      win:'มอเตอร์หมุนอย่างปลอดภัย มีไดโอดคร่อมขดลวดไว้ถูกตำแหน่งแล้ว!'
    })
  },
  {
  ...
  },
  {
    ...
    tutorial:[
      ...
      /* หน้านี้เพิ่มเข้ามาเพราะผู้เล่นเข้าใจผิดกันบ่อย ว่าแค่ "มีตัวเก็บประจุ
         อยู่ในวงจร" แล้วของจะต้องทำงานต่อได้ตอนปิดสวิตช์ — ซึ่งไม่จริง
         มันขึ้นกับว่าต่ออนุกรมหรือขนาน เฉลยของด่านนี้ต่ออนุกรม (กั้นไฟตรง)
         ประจุจึงไหลออกไม่ได้เมื่อสวิตช์เปิดวงจร */
      {img:'Insert_image_here',text:'ต่ออนุกรม กับ ต่อขนาน ให้ผลคนละเรื่องกัน\n\nอนุกรม (แบบด่านนี้) = ตัวเก็บประจุขวางทางไฟ\nไฟวิ่งเข้าไปประจุจนเต็ม แล้วกระแสก็หยุดเอง\nLED จึงสว่างวาบตอนแรกแล้วหรี่ลงจนดับ\nเรียกว่ามันกั้นไฟตรง (บล็อก DC)\nและตอนปิดสวิตช์ วงจรขาดตรงสวิตช์\nประจุที่เก็บไว้ไม่มีทางไหลออก ทุกอย่างดับทันที\n\nขนานคร่อมโหลด = ตัวเก็บประจุเป็นถังสำรอง\nปิดสวิตช์แล้วประจุยังไหลผ่านโหลดได้อยู่\nโหลดจึงทำงานต่ออีกพักหนึ่งแล้วค่อยหรี่ดับ\nนานแค่ไหน:  t = R ของโหลด × C\n220 โอห์ม × 10000 ไมโครฟารัด = 2.2 วินาที\n\nอยากให้ของทำงานต่อตอนตัดไฟ ต้องต่อขนาน\nลองเล่นดูได้ในโหมดอิสระ'},
    ],
    check:levelCheck({
