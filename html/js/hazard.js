...
/* ============================================================
   เดินระบบความร้อนแบบ "ไม่มีกระแสเลย" — ใช้ตอนวงจรเปิด

   วงจรเปิด (สับสวิตช์ OFF · ฟิวส์ขาด · ลบสายทิ้ง) ทำให้ตัวแก้สมการ
   หาคำตอบไม่ได้ (โหนดหลุดออกจากกัน เมทริกซ์เอกฐาน) คืน sol.ok = false
   ของเดิม powerStep() เจอแบบนี้แล้ว return ทิ้งทันที ผลคือ:
     • ความร้อนค้างอยู่ที่ค่าก่อนตัดไฟ ไม่เย็นลงเลย
     • สับสวิตช์กลับมา ON ความร้อนเดินต่อจากเดิม แล้วไหม้อยู่ดี
   ทั้งที่ตัดไฟแล้วต้องเย็นลงตามจริง

   ตัวนี้จึงเดินเฉพาะ "สาขาเย็นลง" ของ hazardStep ให้ครบทุกตัว
   ใช้ HAZARD.COOL เดียวกัน ตัวเลขจึงสอดคล้องกับตอนมีไฟ
   ============================================================ */
function hazardCool(dt){
  HAZARD.t += dt;
  G.wsItems.forEach(function(it){
    if(it.failed) return;              /* พังแล้วก็พังแล้ว ไม่ต้องเย็น */
    it.stress = 0;                     /* ไม่มีกระแส = ไม่มีภาระ */
    it.heat = Math.max(0, (it.heat || 0) - dt / HAZARD.COOL);
  });
  if(!HAZARD.wiresBurned){
    HAZARD.wireHeat = Math.max(0, HAZARD.wireHeat - dt / HAZARD.COOL);
  }
  applyHazardVisuals();                /* เรียก renderHazardBar ให้เองในตัว */
}
